...
import { duplicatePlatformSocialLinks, portfolio } from './public-test-data';
...
describe('PortfolioStore', () => {
  ...
  it('stores duplicate-platform social links as an ordered list', () => { api.getPortfolio.mockReturnValue(of({ ...portfolio, socialLinks: duplicatePlatformSocialLinks })); store.load(); expect(store.data()?.socialLinks.map(link => link.id)).toEqual(['github-coderpl1005', 'github-phucnd3009']); });
