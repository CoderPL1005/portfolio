...
describe('PublicPortfolioService', () => {
  ...
  it('defaults a legacy aggregate without technologies to an empty collection', () => { const { technologies: _, ...legacy } = portfolio; api.get.mockReturnValue(of({ success: true, data: legacy })); service.getPortfolio().subscribe(value => expect(value.technologies).toEqual([])); });
