...
import { formatPortfolioDate, formatPortfolioExactDate, portfolioDurationDays } from '../shared/public-utils';
...
@Component({
  ...
  template: `
    <aside class="inspector" [class.pinned]="pinned()" [class.preview]="!pinned()" [attr.role]="pinned() ? 'dialog' : 'status'" [attr.aria-modal]="pinned() ? 'true' : null" [attr.aria-label]="'Journey details for ' + item().title">
      ...
      @if (durationDays() !== null) { <p class="duration">{{ durationDays() }} days</p> }
  ...
  styles: `
    ...
    p{margin:.4rem 0}.date-range{color:var(--color-primary);font-size:.82rem;font-weight:750}.duration{color:var(--color-text-muted);font-size:.75rem;font-weight:700}.subtitle{color:var(--color-text);font-weight:650}.description{margin-top:.9rem;color:var(--color-text-muted);line-height:1.65}
...
export class JourneyDetailInspectorComponent {
  ...
  date(value: string | null): string | null {
    ...
    const formatter = item.timelineKind === 'PERIOD' && !item.isOngoing && item.startAt && item.endAt
      ? formatPortfolioExactDate
      : formatPortfolioDate;
  ...
  range(): string {
    ...
    const start = this.date(this.item().startAt) ?? this.item().startAt!;
    if (this.item().timelineKind === 'POINT') return start;
    if (this.item().isOngoing) return `${start} → Present`;
    return `${start} → ${this.date(this.item().endAt) ?? 'End date not set'}`;
  }

  durationDays(): number | null {
    const item = this.item();
    if (item.timelineKind !== 'PERIOD' || item.isOngoing) return null;
    return portfolioDurationDays(item.startAt, item.endAt);
  }
}
