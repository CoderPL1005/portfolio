...
const dashboard = {projects:2,experiences:3,skills:4,certificates:5,knowledge:{indexed:0,pending:0,failed:0},conversations:0,recentUpdates:[]};
...
describe('typed settings and social URL safety',()=>{
  beforeEach(()=>TestBed.resetTestingModule());
  it('loads and saves only typed site settings while tracking dirtiness',()=>{const settings={siteName:'Site',footerText:null,showAvailability:true,showDownloadCv:false,showJourney:true,showAiAgent:false,defaultSeoTitle:null,defaultSeoDescription:null};const api={get:vi.fn(()=>of(settings)),update:vi.fn((_value:unknown)=>of(settings))};TestBed.overrideComponent(SiteSettingsPageComponent,{set:{providers:[{provide:SiteSettingsAdminService,useValue:api}]}});TestBed.configureTestingModule({imports:[SiteSettingsPageComponent]});const component=TestBed.createComponent(SiteSettingsPageComponent).componentInstance;component.form.controls.siteName.setValue('Updated');component.save();expect(api.update).toHaveBeenCalledWith(expect.objectContaining({siteName:'Updated'}));expect(Object.keys(api.update.mock.calls[0][0] as object)).toHaveLength(8);expect(component.hasUnsavedChanges()).toBe(false);});
