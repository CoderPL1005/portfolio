        ...
        @if (job(); as current) {
          ...
          <section class="admin-panel detail-card" aria-labelledby="state-title">
            ...
            <div class="state-grid">
              ...
              <label>Selection status<select class="admin-input" [value]="current.selectionStatus" (change)="select(selectValue($event))">@for (status of selections; track status) { <option [value]="status">{{ statusLabel(status) }}</option> }</select></label>
...
export class JobEditPageComponent implements DirtyAware {
  ...
  save(): void {
    ...
    operation.pipe(take(1)).subscribe({
      next: current => { this.form.markAsPristine(); this.sourceForm.markAsPristine(); this.saving.set(false); if (this.isNew) void this.router.navigate(['/admin/job-hunting/jobs', current.id]); else this.job.set(current); },
