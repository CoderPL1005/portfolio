...
import { applicationTransitions, ApplicationDetail, ApplicationPackageReadiness, ApplicationStatus, EventItem } from './job-hunting.models';
    ...
    <div class="admin-page application-workspace">
      ...
      @if (item(); as application) {
        ...
        <section class="admin-panel readiness-card" aria-labelledby="readiness-title">
          <div class="section-heading"><div><p class="section-kicker">APPLICATION PACKAGE</p><h2 id="readiness-title">Package readiness</h2><p>Deterministic checks for the application, approved job, and private canonical CV.</p></div>@if (readiness()) { <button type="button" class="admin-button" [disabled]="readinessLoading()" (click)="loadReadiness()">{{ readinessLoading() ? 'Refreshingâ€¦' : 'Refresh' }}</button> }</div>
          @if (readinessLoading() && !readiness()) { <div class="empty-section" aria-live="polite">Checking package readinessâ€¦</div> }
          @if (readinessError()) { <p class="admin-error" role="alert">{{ readinessError() }}</p> }
...
export class ApplicationDetailPageComponent implements DirtyAware {
  ...
  constructor() { this.load();this.loadReadiness(); }
