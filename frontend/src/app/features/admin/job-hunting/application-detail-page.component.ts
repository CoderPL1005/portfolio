...
import { applicationTransitions, ApplicationDetail, ApplicationPackage, ApplicationPackageReadiness, ApplicationStatus, ApplicationSubmissionReadiness, EventItem, SubmissionAttempt, SubmissionAttemptEvent } from './job-hunting.models';
...
@Component({
  ...
  template: `
    <div class="admin-page application-workspace">
      ...
      @if (item(); as application) {
        ...
        <section class="admin-panel readiness-card" aria-labelledby="readiness-title">
          <div class="section-heading"><div><p class="section-kicker">APPLICATION PACKAGE</p><h2 id="readiness-title">Package readiness</h2><p>Deterministic checks for the application, approved job, and private canonical CV.</p></div>@if (packageState()?.status !== 'FINALIZED' && readiness()) { <button type="button" class="admin-button" [disabled]="readinessLoading() || finalizing()" (click)="refreshPackageState()">{{ readinessLoading() ? 'Refreshing…' : 'Refresh' }}</button> }</div>
          @if ((packageLoading() && !packageState()) || (readinessLoading() && !readiness() && packageState()?.status !== 'FINALIZED')) { <div class="empty-section" aria-live="polite">Checking package readiness…</div> }
          @if (readinessError() || packageError()) { <p class="admin-error" role="alert">{{ readinessError() || packageError() }}</p> }
          @if (packageState()?.status === 'FINALIZED') {
            @if (packageState(); as package) {
              <div class="readiness-summary ready" role="status"><strong>Finalized</strong><span>Package revision {{ package.revision }} · {{ package.finalizedAt | date:'medium' }}</span></div>
              <dl class="cv-summary"><div><dt>Manifest SHA-256</dt><dd>{{ package.manifestHash }}</dd></div>@if (package.cv; as cv) { <div><dt>Immutable CV snapshot</dt><dd>{{ cv.fileName }}</dd></div><div><dt>Integrity</dt><dd>{{ cv.contentType }} · {{ cv.fileSizeBytes }} bytes · canonical CV v{{ cv.sourceCanonicalCvVersion }}</dd></div> }</dl>
              <div class="package-actions"><button type="button" class="admin-button" [disabled]="previewing()" (click)="openPackageCv(false)">{{ previewing() ? 'Opening…' : 'Preview finalized CV' }}</button><button type="button" class="admin-button" [disabled]="downloading()" (click)="openPackageCv(true)">{{ downloading() ? 'Downloading…' : 'Download finalized CV' }}</button></div>
            }
          } @else if (readiness(); as state) {
              <div class="readiness-summary" [class.ready]="state.isReady" role="status"><strong>{{ state.isReady ? 'Ready to finalize' : 'Not ready' }}</strong><span>Application v{{ state.applicationVersion }} · Job v{{ state.jobPostingVersion ?? 'missing' }} · CV v{{ state.canonicalCvVersion ?? 'missing' }}</span></div>
              ...
              @if (state.canonicalCv; as cv) { <dl class="cv-summary"><div><dt>Canonical CV</dt><dd>{{ cv.fileName }}</dd></div><div><dt>Type</dt><dd>{{ cv.contentType }}</dd></div><div><dt>Size</dt><dd>{{ cv.fileSizeBytes }} bytes</dd></div></dl> }
              @if (packageState()?.status === 'DRAFT' && state.isReady && state.jobPostingVersion !== null && state.canonicalCvVersion !== null) { <div class="package-actions"><button type="button" class="admin-button primary" [disabled]="finalizing()" (click)="finalizePackage()">{{ finalizing() ? 'Finalizing…' : 'Finalize package' }}</button></div> }
          }
        </section>
        ...
        <section class="admin-panel submission-card" aria-labelledby="submission-title">
          ...
          <div class="attempt-section">
            <div class="section-heading"><div><h3>Submission attempts</h3><p>Human approval is separate from controlled provider execution.</p></div><button type="button" class="admin-button" [disabled]="attemptsLoading() || attemptAction() !== null" (click)="loadAttempts()">{{ attemptsLoading() ? 'Refreshing…' : 'Refresh attempts' }}</button></div>
            @if (attemptsError()) { <p class="admin-error" role="alert">{{ attemptsError() }}</p> }
            @if (attemptsLoading() && !attempts().length) { <div class="empty-section" aria-live="polite">Loading submission attempts…</div> }
            <div class="attempt-list">
              @for (attempt of attempts(); track attempt.id) {
                <article class="attempt-card">
                  <div class="attempt-heading"><div><strong>{{ statusLabel(attempt.provider) }}</strong><span>Created {{ attempt.createdAt | date:'medium' }}</span></div><span class="status-badge status-{{ attempt.status.toLowerCase() }}">{{ statusLabel(attempt.status) }}</span></div>
                  <dl class="attempt-details"><div><dt>Package</dt><dd>Revision {{ attempt.packageRevision }}</dd></div><div><dt>Attempt version</dt><dd>{{ attempt.version }}</dd></div>@if (approvalEvent(attempt); as approval) { <div><dt>Approved</dt><dd>{{ approval.occurredAt | date:'medium' }}</dd></div> }@if (attempt.startedAt) { <div><dt>Started</dt><dd>{{ attempt.startedAt | date:'medium' }}</dd></div> }@if (attempt.completedAt) { <div><dt>Completed</dt><dd>{{ attempt.completedAt | date:'medium' }}</dd></div> }@if (attempt.providerSubmissionId) { <div><dt>Provider reference</dt><dd>{{ attempt.providerSubmissionId }}</dd></div> }</dl>
                  @if (attempt.failureCode || attempt.failureMessage) { <p class="attempt-result"><strong>{{ attempt.failureCode ? statusLabel(attempt.failureCode) : 'Result details' }}</strong>@if (attempt.failureMessage) { <span>{{ attempt.failureMessage }}</span> }</p> }
                  <ol class="attempt-history">@for (event of attempt.events; track event.id) { <li><span>{{ event.fromStatus ? statusLabel(event.fromStatus) : 'Created' }} → {{ statusLabel(event.toStatus) }}</span><small>{{ event.occurredAt | date:'medium' }}</small></li> }</ol>
                  @if (attempt.status === 'CREATED') { <button type="button" class="admin-button primary" [disabled]="attemptAction() !== null" (click)="approveAttempt(attempt)">{{ attemptAction() === 'approve:'+attempt.id ? 'Approving…' : 'Approve submission' }}</button> }
                  @if (attempt.status === 'APPROVED') { <button type="button" class="admin-button primary" [disabled]="attemptAction() !== null" (click)="executeAttempt(attempt)">{{ attemptAction() === 'execute:'+attempt.id ? 'Executing…' : 'Execute approved submission' }}</button> }
                  @if (attempt.status === 'SUBMITTING') { <p class="attempt-result"><strong>Execution in progress or awaiting reconciliation.</strong><span>No automatic retry is performed.</span></p> }
                  @if (attempt.provider === 'MANUAL' && attempt.status === 'APPROVED') { <p class="terminal-copy">MANUAL has no production submission adapter. Execution will stop safely unless an adapter is explicitly configured.</p> }
                </article>
              } @empty { @if (!attemptsLoading()) { <div class="empty-section"><strong>No submission attempts.</strong><span>No provider execution has been requested.</span></div> } }
            </div>
          </div>
        </section>
  ...
  styles: `
    ...
    .readiness-summary{display:flex;justify-content:space-between;gap:1rem;padding:1rem;border:1px solid color-mix(in srgb,#f59e0b 45%,var(--color-border));border-radius:var(--radius-md);background:color-mix(in srgb,#f59e0b 8%,var(--color-surface-lowest))}.readiness-summary.ready{border-color:color-mix(in srgb,#22c55e 45%,var(--color-border));background:color-mix(in srgb,#22c55e 8%,var(--color-surface-lowest))}.readiness-summary span{color:var(--color-text-muted)}.blocker-list{display:grid;gap:.65rem;margin:1rem 0 0;padding:0;list-style:none}.blocker-list li{display:grid;gap:.25rem;padding:.8rem 1rem;border-left:3px solid #f59e0b;background:var(--color-surface-lowest)}.blocker-list span{color:var(--color-text-muted)}.cv-summary{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.75rem;margin:1rem 0 0}.cv-summary div{padding:.75rem;border:1px solid var(--color-border);border-radius:var(--radius-md)}.cv-summary dt{color:var(--color-text-dim);font-size:.7rem;text-transform:uppercase}.cv-summary dd{margin:.3rem 0 0;overflow-wrap:anywhere}.package-actions{display:flex;gap:.65rem;flex-wrap:wrap;margin-top:1rem}.attempt-section{display:grid;gap:1rem;margin-top:1.25rem;padding-top:1.25rem;border-top:1px solid var(--color-border)}.attempt-list{display:grid;gap:.85rem}.attempt-card{display:grid;gap:.85rem;padding:1rem;border:1px solid var(--color-border);border-radius:var(--radius-lg);background:var(--color-surface-lowest)}.attempt-heading{display:flex;justify-content:space-between;gap:1rem;align-items:flex-start}.attempt-heading span,.attempt-history small{color:var(--color-text-muted)}.attempt-details{display:flex;gap:1rem;flex-wrap:wrap;margin:0}.attempt-details dt{color:var(--color-text-dim);font-size:.7rem;text-transform:uppercase}.attempt-history{display:grid;gap:.35rem;margin:0;padding-left:1.25rem}.attempt-history li{display:flex;justify-content:space-between;gap:1rem}.attempt-result{display:grid;gap:.25rem;margin:0;padding:.75rem;border-left:3px solid #f59e0b;background:var(--color-surface)}.attempt-result span{color:var(--color-text-muted)}
    ...
    @media(max-width:760px){.workspace-header,.section-heading{align-items:stretch;flex-direction:column}.header-actions{justify-content:flex-start}.stage-card,.details-grid,.form-grid,.cv-summary{grid-template-columns:1fr}.details-grid .wide,.form-grid .wide{grid-column:auto}.transition-actions{display:grid}.transition-actions .danger{margin-left:0}.document-card,.readiness-summary{align-items:flex-start;flex-direction:column}.document-card .admin-button{width:100%}.progress-track li{font-size:.7rem}.admin-panel{padding:1rem}}
...
export class ApplicationDetailPageComponent implements DirtyAware {
  ...
  readonly attempts = signal<SubmissionAttempt[]>([]); readonly attemptsLoading = signal(true); readonly attemptsError = signal<string | null>(null); readonly attemptAction = signal<string | null>(null);
  ...
  constructor() { this.load();this.loadReadiness();this.loadPackage();this.loadSubmissionReadiness();this.loadAttempts(); }
  ...
  loadAttempts() { this.attemptsLoading.set(true);this.attemptsError.set(null);this.api.submissionAttempts(this.id).pipe(take(1)).subscribe({next:attempts=>{this.attempts.set(attempts);this.attemptsLoading.set(false)},error:error=>{this.attemptsError.set(safeAdminError(error));this.attemptsLoading.set(false)}}); }
  ...
  approveAttempt(attempt:SubmissionAttempt) { if(this.attemptAction()||attempt.status!=='CREATED'||!confirm('Approve this submission attempt? Approval does not execute the provider.'))return;this.attemptAction.set('approve:'+attempt.id);this.attemptsError.set(null);this.api.approveSubmissionAttempt(attempt.id,attempt.version).pipe(take(1)).subscribe({next:updated=>{this.replaceAttempt(updated);this.attemptAction.set(null)},error:error=>this.attemptFailed(error)}); }
  executeAttempt(attempt:SubmissionAttempt) { if(this.attemptAction()||attempt.status!=='APPROVED'||!confirm('Execute this approved submission through its configured provider? This can cause an external submission.'))return;this.attemptAction.set('execute:'+attempt.id);this.attemptsError.set(null);this.api.executeSubmissionAttempt(attempt.id,attempt.version).pipe(take(1)).subscribe({next:updated=>{this.replaceAttempt(updated);this.attemptAction.set(null);this.load();this.loadSubmissionReadiness()},error:error=>this.attemptFailed(error)}); }
  approvalEvent(attempt:SubmissionAttempt):SubmissionAttemptEvent|null { return attempt.events.find(event=>event.fromStatus==='CREATED'&&event.toStatus==='APPROVED')??null; }
  ...
  private replaceAttempt(updated:SubmissionAttempt) { this.attempts.update(items=>items.map(item=>item.id===updated.id?updated:item)); }
  private attemptFailed(error:unknown) { this.attemptsError.set(safeAdminError(error));this.attemptAction.set(null);if((error as {status?:number})?.status===409)this.loadAttempts(); }
