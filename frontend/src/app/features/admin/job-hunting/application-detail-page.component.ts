        ...
        <section class="admin-panel readiness-card" aria-labelledby="readiness-title">
          ...
          @if (packageState()?.status === 'FINALIZED') {
            @if (packageState(); as package) {
              <div class="readiness-summary ready" role="status"><strong>Finalized</strong><span>Package revision {{ package.revision }} Â· {{ package.finalizedAt | date:'medium' }}</span></div>
              <dl class="cv-summary"><div><dt>Manifest SHA-256</dt><dd>{{ package.manifestHash }}</dd></div>@if (package.cv; as cv) { <div><dt>Immutable CV snapshot</dt><dd>{{ cv.fileName }}</dd></div><div><dt>Integrity</dt><dd>{{ cv.contentType }} Â· {{ cv.fileSizeBytes }} bytes Â· canonical CV v{{ cv.sourceCanonicalCvVersion }}</dd></div> }</dl>
              <div class="package-actions"><button type="button" class="admin-button" [disabled]="previewing()" (click)="openPackageCv(false)">{{ previewing() ? 'Openingâ€¦' : 'Preview finalized CV' }}</button><button type="button" class="admin-button" [disabled]="downloading()" (click)="openPackageCv(true)">{{ downloading() ? 'Downloadingâ€¦' : 'Download finalized CV' }}</button></div>
            }
          } @else if (readiness(); as state) {
              <div class="readiness-summary" [class.ready]="state.isReady" role="status"><strong>{{ state.isReady ? 'Ready to finalize' : 'Not ready' }}</strong><span>Application v{{ state.applicationVersion }} Â· Job v{{ state.jobPostingVersion ?? 'missing' }} Â· CV v{{ state.canonicalCvVersion ?? 'missing' }}</span></div>
              ...
              @if (packageState()?.status === 'DRAFT' && state.isReady && state.jobPostingVersion !== null && state.canonicalCvVersion !== null) { <div class="package-actions"><button type="button" class="admin-button primary" [disabled]="finalizing()" (click)="finalizePackage()">{{ finalizing() ? 'Finalizingâ€¦' : 'Finalize package' }}</button></div> }
          }
        </section>
