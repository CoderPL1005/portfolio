...
import { ApplicationDetail, ApplicationItem, ApplicationPackage, ApplicationPackageReadiness, ApplicationSubmissionReadiness, JobDetail, JobFitAnalysis, JobSummary, SubmissionAttempt } from './job-hunting.models';
...
const submissionReadiness=(overrides:Partial<ApplicationSubmissionReadiness>={}):ApplicationSubmissionReadiness=>({status:'READY_FOR_SUBMISSION',isReady:true,blockers:[],package:{applicationId:'app-1',jobPostingId:'job-1',applicationVersion:2,packageRevision:1,packageManifestHash:'b'.repeat(64),cvDocumentId:'doc-1',cvFileName:'cv.pdf',cvContentType:'application/pdf',cvFileSizeBytes:1024,cvContentHash:'a'.repeat(64)},...overrides});
const submissionAttempt=(overrides:Partial<SubmissionAttempt>={}):SubmissionAttempt=>({id:'attempt-1',jobApplicationId:'app-1',provider:'MANUAL',status:'CREATED',packageRevision:1,packageManifestHash:'b'.repeat(64),applicationVersionAtCreation:2,createdAt:'2026-09-23T01:00:00Z',createdByAdminUserId:'admin-1',startedAt:null,completedAt:null,providerSubmissionId:null,failureCode:null,failureMessage:null,version:1,events:[{id:'attempt-event-1',fromStatus:null,toStatus:'CREATED',actorAdminUserId:'admin-1',occurredAt:'2026-09-23T01:00:00Z',createdAt:'2026-09-23T01:00:00Z'}],...overrides});

function mount<T>(component:new(...args:any[])=>T,api:Record<string,any>,id?:string):ComponentFixture<T>{
  const service={packageReadiness:()=>of(packageReadiness()),submissionReadiness:()=>of(submissionReadiness()),submissionAttempts:()=>of([]),applicationPackage:()=>of(applicationPackage()),...api};
...
describe('Application detail workflows',()=>{
  afterEach(()=>{vi.restoreAllMocks();TestBed.resetTestingModule()});
  it.each([
    ['DRAFT',['Record external submission','Withdraw']],['APPLIED',['Move to interview','Reject','Withdraw']],['INTERVIEW',['Reject','Mark offer received','Withdraw']],['OFFER',['Withdraw']],
  ...
  it('renders draft-package submission blockers without presenting an applied action',()=>{
    const state=submissionReadiness({status:'NOT_READY_FOR_SUBMISSION',isReady:false,blockers:[{code:'PACKAGE_NOT_FINALIZED',message:'Finalize the immutable application package before recording a submission.'}],package:null});const fixture=mount(ApplicationDetailPageComponent,{application:()=>of(applicationDetail()),submissionReadiness:()=>of(state)},'app-1');const text=fixture.nativeElement.textContent as string;expect(text).toContain('Not ready for submission');expect(text).toContain('Package Not Finalized');expect(text).not.toContain('Record external submission');
  });
  it('renders ready handoff and unambiguous manual external-submission copy',()=>{
    const fixture=mount(ApplicationDetailPageComponent,{application:()=>of(applicationDetail())},'app-1');const text=fixture.nativeElement.textContent as string;expect(text).toContain('Ready for submission');expect(text).toContain('This application has not been submitted by this system');expect(text).toContain('Record external submission');expect(text).not.toContain('Submit application');
  });
  it('renders and explicitly approves a created attempt without executing it',()=>{
    vi.spyOn(window,'confirm').mockReturnValue(true);const approved=submissionAttempt({status:'APPROVED',version:2,events:[...submissionAttempt().events,{id:'event-2',fromStatus:'CREATED',toStatus:'APPROVED',actorAdminUserId:'admin-1',occurredAt:'2026-09-23T02:00:00Z',createdAt:'2026-09-23T02:00:00Z'}]});const api={application:()=>of(applicationDetail()),submissionAttempts:()=>of([submissionAttempt()]),approveSubmissionAttempt:vi.fn(()=>of(approved)),executeSubmissionAttempt:vi.fn()};const fixture=mount(ApplicationDetailPageComponent,api,'app-1');const c=fixture.componentInstance;expect(fixture.nativeElement.textContent).toContain('Approve submission');c.approveAttempt(c.attempts()[0]);fixture.detectChanges();expect(api.approveSubmissionAttempt).toHaveBeenCalledWith('attempt-1',1);expect(api.executeSubmissionAttempt).not.toHaveBeenCalled();expect(c.attempts()[0].status).toBe('APPROVED');expect(fixture.nativeElement.textContent).toContain('Execute approved submission');expect(fixture.nativeElement.textContent).toContain('MANUAL has no production submission adapter');
  });
  it('confirms controlled execution once and renders the server-owned result',()=>{
    vi.spyOn(window,'confirm').mockReturnValue(true);const approved=submissionAttempt({provider:'EMAIL',status:'APPROVED',version:2});const pending=new Subject<SubmissionAttempt>();const api={application:vi.fn(()=>of(applicationDetail())),submissionAttempts:()=>of([approved]),executeSubmissionAttempt:vi.fn(()=>pending)};const fixture=mount(ApplicationDetailPageComponent,api,'app-1');const c=fixture.componentInstance;c.executeAttempt(approved);c.executeAttempt(approved);fixture.detectChanges();expect(api.executeSubmissionAttempt).toHaveBeenCalledTimes(1);expect(api.executeSubmissionAttempt).toHaveBeenCalledWith('attempt-1',2);expect(fixture.nativeElement.textContent).toContain('Executing');pending.next(submissionAttempt({provider:'EMAIL',status:'FAILED',version:4,startedAt:'2026-09-23T03:00:00Z',completedAt:'2026-09-23T03:01:00Z',failureCode:'PROVIDER_FAILURE',failureMessage:'Rejected safely'}));pending.complete();fixture.detectChanges();expect(c.attemptAction()).toBeNull();expect(fixture.nativeElement.textContent).toContain('Provider Failure');expect(fixture.nativeElement.textContent).toContain('Rejected safely');
  });
  it('does not call approval or execution when confirmation is declined',()=>{
    vi.spyOn(window,'confirm').mockReturnValue(false);const created=submissionAttempt();const approved=submissionAttempt({id:'attempt-2',status:'APPROVED',version:2});const api={application:()=>of(applicationDetail()),submissionAttempts:()=>of([created,approved]),approveSubmissionAttempt:vi.fn(),executeSubmissionAttempt:vi.fn()};const fixture=mount(ApplicationDetailPageComponent,api,'app-1');fixture.componentInstance.approveAttempt(created);fixture.componentInstance.executeAttempt(approved);expect(api.approveSubmissionAttempt).not.toHaveBeenCalled();expect(api.executeSubmissionAttempt).not.toHaveBeenCalled();
  });
  it('does not present the recording action for a stale handoff version',()=>{
    const stale=submissionReadiness({package:{...submissionReadiness().package!,applicationVersion:3}});const fixture=mount(ApplicationDetailPageComponent,{application:()=>of(applicationDetail({version:2})),submissionReadiness:()=>of(stale)},'app-1');expect(fixture.nativeElement.textContent).not.toContain('Record external submission');
  });
  it('does not invite another submission after applied',()=>{
    const fixture=mount(ApplicationDetailPageComponent,{application:()=>of(applicationDetail({status:'APPLIED'})),submissionReadiness:()=>of(submissionReadiness({status:'NOT_READY_FOR_SUBMISSION',isReady:false,blockers:[{code:'APPLICATION_NOT_DRAFT',message:'Only a DRAFT application can be prepared for a new submission.'}],package:null}))},'app-1');const text=fixture.nativeElement.textContent as string;expect(text).toContain('Submission already recorded');expect(text).not.toContain('Record external submission');
  });
  ...
  it('transitions once while pending with expectedVersion and the optional note',()=>{
    const confirm=vi.spyOn(window,'confirm').mockReturnValue(true);const pending=new Subject<ApplicationDetail>();const api={application:()=>of(applicationDetail()),packageReadiness:vi.fn(()=>of(packageReadiness())),submissionReadiness:vi.fn(()=>of(submissionReadiness())),transition:vi.fn(()=>pending)};const fixture=mount(ApplicationDetailPageComponent,api,'app-1');const c=fixture.componentInstance;c.transitionNote.setValue('Sent');c.transition('APPLIED');c.transition('APPLIED');fixture.detectChanges();expect(api.transition).toHaveBeenCalledTimes(1);expect(api.transition).toHaveBeenCalledWith('app-1',{status:'APPLIED',expectedVersion:2,note:'Sent'});expect(confirm).toHaveBeenCalledWith('Confirm this application was already submitted outside this system?');expect([...fixture.nativeElement.querySelectorAll('.transition-actions button')].every((button:HTMLButtonElement)=>button.disabled)).toBe(true);pending.next(applicationDetail({version:3,status:'APPLIED'}));pending.complete();expect(c.item()?.version).toBe(3);expect(c.transitionPending()).toBeNull();expect(api.packageReadiness).toHaveBeenCalledTimes(2);expect(api.submissionReadiness).toHaveBeenCalledTimes(2);
  });
