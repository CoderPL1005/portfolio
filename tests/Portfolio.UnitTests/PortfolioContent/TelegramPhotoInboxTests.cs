...
public sealed class TelegramPhotoInboxTests
{
    ...
    public async Task Album_parts_share_one_raw_job_order_by_message_and_never_acknowledge_per_item()
    {
        ...
        Assert.Equal("telegram-album:200:album-A",raw.IngestionKey);Assert.Equal("Album caption",raw.RawContent);Assert.Equal(5,raw.Version);Assert.Equal(new long?[]{10,20},attachments.Select(x=>x.TelegramMessageId));Assert.Equal(2,attachments.Count);Assert.Equal(2,telegram.DownloadedFileIds.Count);Assert.Empty(telegram.Messages);
