...
public sealed class AgentFeatureTests
{
    ...
    [Theory][InlineData(false)][InlineData(true)]public async Task Chat_sanitizes_embedding_failures_and_dimension_mismatches(bool wrongDimension){await using var db=PublicPortfolioTests.CreateContext();var s=Session();db.AddRange(Setting(),s);await db.SaveChangesAsync();IEmbeddingService embedding=wrongDimension?new WrongDimensionEmbedding():new ThrowingEmbedding();var error=await Assert.ThrowsAsync<ServiceUnavailableException>(()=>new SendChatMessageCommandHandler(db,new AllowQuota(),embedding,new EmptyRetriever(),new FakeChat(),new FixedTimeProvider(Now)).HandleAsync(new(s.PublicSessionId,"Hi","ip:test")));Assert.Equal("AGENT_UNAVAILABLE",error.Code);Assert.DoesNotContain("embedding",error.Message,StringComparison.OrdinalIgnoreCase);Assert.Empty(await db.ChatMessages.ToListAsync());}
    [Fact]public async Task Chat_sanitizes_completion_provider_failure(){await using var db=PublicPortfolioTests.CreateContext();var s=Session();var doc=Document();var chunk=Chunk(doc.Id);db.AddRange(Setting(),s,doc,chunk);await db.SaveChangesAsync();var error=await Assert.ThrowsAsync<ServiceUnavailableException>(()=>new SendChatMessageCommandHandler(db,new AllowQuota(),new FakeEmbedding(),new FakeRetriever(chunk.Id,doc.Id),new ThrowingChat(),new FixedTimeProvider(Now)).HandleAsync(new(s.PublicSessionId,"Hi","ip:test")));Assert.Equal("AGENT_UNAVAILABLE",error.Code);Assert.Empty(await db.ChatMessages.ToListAsync());}
    [Theory]
    ...
    public async Task Quota_rejection_happens_before_provider_calls_or_message_persistence(string code)
    {
        ...
        var handler = new SendChatMessageCommandHandler(
            db, new RejectQuota(code), embedding, new EmptyRetriever(), completion, new FixedTimeProvider(Now));
