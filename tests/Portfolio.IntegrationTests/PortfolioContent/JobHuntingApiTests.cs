...
public sealed class JobHuntingApiTests(AuthApiFactory root):IClassFixture<AuthApiFactory>
{
    ...
    [Fact]public async Task Submission_readiness_is_http_200_and_exposes_only_safe_handoff_metadata()
    {
        using var factory=CreateFactory();using var client=Authenticated(factory);var response=await client.GetAsync($"/api/v1/admin/job-hunting/applications/{Fakes.ApplicationId}/submission-readiness");Assert.Equal(HttpStatusCode.OK,response.StatusCode);var data=(await response.Content.ReadFromJsonAsync<JsonElement>()).GetProperty("data");Assert.Equal("READY_FOR_SUBMISSION",data.GetProperty("status").GetString());Assert.True(data.GetProperty("isReady").GetBoolean());var package=data.GetProperty("package");Assert.Equal(Fakes.ApplicationId,package.GetProperty("applicationId").GetGuid());Assert.False(package.ToString().Contains("storage",StringComparison.OrdinalIgnoreCase));Assert.False(package.ToString().Contains("url",StringComparison.OrdinalIgnoreCase));
    }
    ...
    private WebApplicationFactory<Program> CreateFactory()=>root.WithWebHostBuilder(builder=>builder.ConfigureServices(services=>
    {
        ...
        Replace<GetApplicationSubmissionReadinessQuery,ApplicationSubmissionReadinessResult,Fakes>(services);
    ...
    public sealed class Fakes:IRequestHandler<GetJobPostingsQuery,PagedResult<JobPostingListItem>>,IRequestHandler<GetJobPostingQuery,JobPostingResult>,IRequestHandler<CreateJobPostingCommand,JobPostingResult>,IRequestHandler<ArchiveJobPostingCommand,JobPostingResult>,IRequestHandler<GetJobApplicationsQuery,PagedResult<JobApplicationListItem>>,IRequestHandler<GetJobApplicationQuery,JobApplicationResult>,IRequestHandler<CreateJobApplicationCommand,JobApplicationResult>,IRequestHandler<GetApplicationPackageReadinessQuery,ApplicationPackageReadinessResult>,IRequestHandler<GetApplicationSubmissionReadinessQuery,ApplicationSubmissionReadinessResult>,IRequestHandler<GetApplicationPackageQuery,ApplicationPackageResult>,IRequestHandler<FinalizeApplicationPackageCommand,ApplicationPackageResult>,IRequestHandler<GetApplicationPackageContentQuery,ApplicationPackageContentResult>,IRequestHandler<GetCandidateJobPreferencesQuery,CandidateJobPreferencesResult>,IRequestHandler<UpdateCandidateJobPreferencesCommand,CandidateJobPreferencesResult>,IRequestHandler<GetJobFitAnalysisQuery,JobFitAnalysisResult>
    {
        ...
        public Task<ApplicationSubmissionReadinessResult> HandleAsync(GetApplicationSubmissionReadinessQuery r,CancellationToken ct=default)=>Task.FromResult(new ApplicationSubmissionReadinessResult("READY_FOR_SUBMISSION",true,[],new(ApplicationId,JobId,1,1,new string('b',64),Guid.NewGuid(),"canonical.pdf","application/pdf",1024,new string('a',64))));
