...
public sealed record CreateChatSessionCommand:IRequest<ChatSessionResult>;
public sealed record ChatSessionResult(Guid SessionId,string Status,string? WelcomeMessage);
public sealed record SendChatMessageCommand(Guid SessionId,string Message,string VisitorKey):IRequest<ChatAnswerResult>;
public sealed record ChatSourceResult(string Title,string SourceType,Guid? SourceRefId,string? ProjectSlug,int Rank,decimal SimilarityScore);
...
public sealed record GetConversationsQuery(int Page=1,int PageSize=20,string? Status=null):IRequest<PagedResult<ConversationListItem>>;
public sealed record ConversationListItem(Guid Id,Guid PublicSessionId,string Status,DateTimeOffset StartedAt,DateTimeOffset? LastMessageAt,int MessageCount);
public sealed record ConversationSource(string Title,int Rank,decimal? SimilarityScore);
public sealed record ConversationFeedback(string Rating,string? Comment);
public sealed record ConversationMessage(Guid Id,string Role,string Content,string? ModelName,int? PromptTokens,int? CompletionTokens,int? LatencyMs,DateTimeOffset CreatedAt,IReadOnlyCollection<ConversationSource> Sources,ConversationFeedback? Feedback);
public sealed record ConversationDetail(Guid Id,Guid PublicSessionId,string Status,DateTimeOffset StartedAt,DateTimeOffset? LastMessageAt,int MessageCount,IReadOnlyCollection<ConversationMessage> Messages);
public sealed record GetConversationQuery(Guid Id):IRequest<ConversationDetail>;public sealed record CloseConversationCommand(Guid Id):IRequest<bool>;
public sealed class GetConversationsQueryHandler(IApplicationDbContext db):IRequestHandler<GetConversationsQuery,PagedResult<ConversationListItem>>{public async Task<PagedResult<ConversationListItem>> HandleAsync(GetConversationsQuery r,CancellationToken ct=default){var q=db.ChatSessions.AsNoTracking();if(!string.IsNullOrWhiteSpace(r.Status)){var s=r.Status.Trim().ToUpper();q=q.Where(x=>x.Status==s);}var total=await q.CountAsync(ct);var items=await q.OrderByDescending(x=>x.LastMessageAt??x.StartedAt).ThenBy(x=>x.Id).Skip((r.Page-1)*r.PageSize).Take(r.PageSize).Select(x=>new ConversationListItem(x.Id,x.PublicSessionId,x.Status,x.StartedAt,x.LastMessageAt,x.MessageCount)).ToListAsync(ct);return new(items,r.Page,r.PageSize,total);}}
public sealed class GetConversationsQueryValidator:IRequestValidator<GetConversationsQuery>{public Task<IReadOnlyCollection<ValidationFailure>> ValidateAsync(GetConversationsQuery r,CancellationToken ct=default){var e=new List<ValidationFailure>();if(r.Page<1)e.Add(new("page","Page must be at least 1."));if(r.PageSize is<1 or>100)e.Add(new("pageSize","Page size must be between 1 and 100."));if(r.Status is not null&&!new[]{"ACTIVE","CLOSED"}.Contains(r.Status.Trim().ToUpper()))e.Add(new("status","Status is invalid."));return Task.FromResult<IReadOnlyCollection<ValidationFailure>>(e);}}
public sealed class GetConversationQueryHandler(IApplicationDbContext db):IRequestHandler<GetConversationQuery,ConversationDetail>{public async Task<ConversationDetail> HandleAsync(GetConversationQuery r,CancellationToken ct=default){var s=await db.ChatSessions.AsNoTracking().SingleOrDefaultAsync(x=>x.Id==r.Id,ct)??throw new NotFoundException("CHAT_SESSION_NOT_FOUND","Chat session was not found.");var messages=await db.ChatMessages.AsNoTracking().Where(x=>x.ChatSessionId==s.Id).OrderBy(x=>x.CreatedAt).ThenBy(x=>x.Id).ToListAsync(ct);var result=new List<ConversationMessage>();foreach(var m in messages){var sources=await db.ChatMessageSources.AsNoTracking().Where(x=>x.ChatMessageId==m.Id).OrderBy(x=>x.Rank).Select(x=>new ConversationSource(x.KnowledgeChunk.KnowledgeDocument.Title,x.Rank,x.SimilarityScore)).ToListAsync(ct);var f=await db.ChatMessageFeedback.AsNoTracking().Where(x=>x.ChatMessageId==m.Id).Select(x=>new ConversationFeedback(x.Rating,x.Comment)).SingleOrDefaultAsync(ct);result.Add(new(m.Id,m.Role,m.Content,m.ModelName,m.PromptTokens,m.CompletionTokens,m.LatencyMs,m.CreatedAt,sources,f));}return new(s.Id,s.PublicSessionId,s.Status,s.StartedAt,s.LastMessageAt,s.MessageCount,result);}}
public sealed class CloseConversationCommandHandler(IApplicationDbContext db,TimeProvider clock):IRequestHandler<CloseConversationCommand,bool>{public async Task<bool> HandleAsync(CloseConversationCommand r,CancellationToken ct=default){var x=await db.ChatSessions.SingleOrDefaultAsync(x=>x.Id==r.Id,ct)??throw new NotFoundException("CHAT_SESSION_NOT_FOUND","Chat session was not found.");x.Status="CLOSED";x.ClosedAt=clock.GetUtcNow();await db.SaveChangesAsync(ct);return true;}}
