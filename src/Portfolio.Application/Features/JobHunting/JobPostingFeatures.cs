...
{public async Task<JobPostingResult> HandleAsync(UpdateJobSelectionCommand r,CancellationToken ct=default){var x=await JobPostingMapping.RequireVersion(db,r.Id,r.ExpectedVersion,ct);var target=r.Status.Trim().ToUpperInvariant();if(!JobSelectionTransitionPolicy.CanTransition(x.SelectionStatus,target))throw new ConflictException("JOB_SELECTION_TRANSITION_INVALID",$"Transition from {x.SelectionStatus} to {target} is not allowed.");x.SelectionStatus=target;x.Version++;x.UpdatedAt=clock.GetUtcNow();await JobPostingMapping.SaveVersioned(db,ct);return await JobPostingMapping.GetAsync(db,x.Id,ct);}}
...
internal static class JobValidation
{
    ...
    public static readonly string[] HumanSelection=["APPROVED","SKIPPED"];
    ...
    public static void Email(List<ValidationFailure> f,string p,string? v){Optional(f,p,v,255);if(!string.IsNullOrWhiteSpace(v)&&(!System.Net.Mail.MailAddress.TryCreate(v.Trim(),out var a)||!string.Equals(a.Address,v.Trim(),StringComparison.OrdinalIgnoreCase)))f.Add(new(p,$"{p} must be a valid email address."));}
    private static void Numeric18Scale2(List<ValidationFailure> f,string p,decimal? v){const decimal maximum=9999999999999999.99m;if(v.HasValue&&(v.Value>maximum||decimal.Round(v.Value,2)!=v.Value))f.Add(new(p,$"{p} must fit numeric(18,2)."));}
}
