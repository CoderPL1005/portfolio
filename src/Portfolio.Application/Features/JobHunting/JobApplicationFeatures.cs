...
public sealed class TransitionJobApplicationCommandHandler(IApplicationDbContext db,TimeProvider clock,ICurrentUser currentUser):IRequestHandler<TransitionJobApplicationCommand,JobApplicationResult>
{
    public async Task<JobApplicationResult> HandleAsync(TransitionJobApplicationCommand r,CancellationToken ct=default){var x=await JobApplicationMapping.RequireVersion(db,r.Id,r.ExpectedVersion,ct);var target=r.Status.Trim().ToUpperInvariant();if(!JobApplicationTransitionPolicy.CanTransition(x.Status,target))throw new ConflictException("JOB_APPLICATION_TRANSITION_INVALID",$"Transition from {x.Status} to {target} is not allowed.");var now=clock.GetUtcNow();var occurred=r.OccurredAt??now;if(occurred>now)throw new ValidationException([new("occurredAt","Occurred time cannot be in the future.")]);var from=x.Status;x.Status=target;if(target==JobApplicationStatuses.Applied&&x.AppliedAt is null)x.AppliedAt=occurred;x.LastActivityAt=occurred;x.Version++;x.UpdatedAt=now;db.JobApplicationEvents.Add(JobApplicationMapping.Event(x.Id,JobApplicationEventTypes.StatusChanged,from,target,currentUser.AdminUserId,JobText.TrimOrNull(r.Note),occurred,now));await JobApplicationMapping.SaveVersioned(db,ct);return await JobApplicationMapping.GetAsync(db,x.Id,ct);}
}
