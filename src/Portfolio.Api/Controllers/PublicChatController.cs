...
[ApiController,Route("api/v1/public/chat"),EnableRateLimiting("public-chat")]
public sealed class PublicChatController(IRequestDispatcher dispatcher):ControllerBase
{
    ...
    [HttpPost("sessions/{sessionId:guid}/messages")]public async Task<ActionResult<ApiResponse<ChatAnswerResult>>> Message(Guid sessionId,ChatMessageRequest request,CancellationToken ct)=>Ok(ApiResponse<ChatAnswerResult>.Ok(await dispatcher.DispatchAsync(new SendChatMessageCommand(sessionId,request.Message),ct)));
