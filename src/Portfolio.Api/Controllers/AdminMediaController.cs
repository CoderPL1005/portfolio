...
public sealed class AdminMediaController(IRequestDispatcher dispatcher) : ControllerBase
{
    ...
    [HttpPost, RequestSizeLimit(10 * 1024 * 1024 + 65536)] public async Task<ActionResult<ApiResponse<MediaAssetResult>>> Upload([FromForm] IFormFile file, [FromForm] string mediaType, [FromForm] string? altText, CancellationToken ct) { await using var stream = file.OpenReadStream(); var result = await dispatcher.DispatchAsync(new UploadMediaCommand(stream, file.FileName, file.ContentType, file.Length, mediaType, altText), ct); return StatusCode(StatusCodes.Status201Created, ApiResponse<MediaAssetResult>.Ok(result)); }
