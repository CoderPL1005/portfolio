...
## PostgreSQL / Neon — Schema V1 Final
...
-- 10. PROJECT SECTIONS
--
-- Flexible structure used by all Project Detail pages.
-- Avoids creating separate tables for every section type.
-- ============================================================
...
-- 18. SITE SETTINGS
--
-- Flexible small configuration values.
-- Do NOT put core portfolio entities here.
-- ============================================================
...
-- Secrets/API keys MUST stay in environment variables,
-- never in this table.
-- ============================================================
...
--
-- Derived searchable representation of portfolio data.
--
-- source_ref_id is intentionally polymorphic.
-- FK is NOT used because it may reference different source tables.
-- ============================================================
...
-- VECTOR(1536)
--
-- Therefore V1 must use an embedding model producing
...
--
-- Citation / grounding records for ASSISTANT answers.
-- ============================================================
...
--
-- Separated from chat_messages because feedback is a different
-- user action and may be created after the answer itself.
-- ============================================================
...
# Quy tắc kiến trúc DB V1

**Portfolio là source of truth.**
...
Không được thiết kế theo chiều ngược lại:
...
Knowledge chỉ là dữ liệu dẫn xuất để chatbot search.

# Quy tắc khi Admin cập nhật
...
Edit SchoolSaaS
 ↓
projects/project_sections updated
 ↓
transaction commit
 ↓
knowledge document marked PENDING
 ↓
knowledge worker
 ↓
regenerate content
...
Website vẫn hoạt động bình thường vì portfolio data không phụ thuộc RAG.

# Mapping Admin CMS

Schema cuối này tương ứng trực tiếp với:
...
# Những thứ cố tình không có trong V1
...
Bạn chỉ có **một admin**, nên không cần RBAC cho admin CMS.

Dữ liệu portfolio nhỏ nên không cần Redis.

RAG nhỏ nên PostgreSQL + pgvector là đủ.

# Chốt stack đi cùng schema
...
Nếu trong quá trình code không phát hiện requirement nghiệp vụ mới thực sự bắt buộc thì **không đổi domain/schema nữa**; các thay đổi implementation nhỏ phải được xử lý trong EF Core/config/service thay vì tiếp tục phình database.
